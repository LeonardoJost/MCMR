...
cohensDsim=function(ns,numberOfEachTrials=c(8,12,16),reps=100,sd=0.73,sd2=0.34){
  numTrials=length(numberOfEachTrials)
  #loop over number of participants
  dsDataFrame=data.frame(numberOfEachTrial=rep(numberOfEachTrials,each=2*length(ns)),
                                  n=rep(ns,each=2*numTrials),
                                  probType=rep(c("logOdds","responses"),length(ns)*numTrials),
                                  value=rep(0,length(ns)*2*numTrials))
  for(numberOfEachTrial in numberOfEachTrials){
    for(n in ns) {
      #every combination (3 nStimuli*2 types=6) twice
      N=n*12
      cohensDs = matrix(nrow=reps, ncol=2)
      for(i in 1:reps){
        testdata=generateData(n,N,numberOfEachTrial,sd,sd2)
...
set.seed(88735)
numberOfEachTrials=c(8,12,16,24,48)
